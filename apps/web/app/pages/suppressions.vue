<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-display text-stone-900 dark:text-stone-100">Suppressions</h1>
        <p class="mt-1 text-stone-500 dark:text-stone-400">Manage email addresses that should not receive messages.</p>
      </div>
      <span v-if="suppressions" class="text-sm text-stone-500 dark:text-stone-400">{{ formatNumber(suppressions.total) }} total</span>
    </div>
    ...
    <div class="card-elevated p-6">
      ...
      <template v-else-if="suppressions">
        <div v-if="suppressions.items.length === 0" class="empty-state">
          <div class="empty-state-icon">
            <UIcon name="i-heroicons-check-circle" class="w-8 h-8 text-stone-400" />
          </div>
          <p class="text-stone-500 dark:text-stone-400">No suppressions</p>
          <p class="text-sm text-stone-400 dark:text-stone-500 mt-1">All emails are being delivered successfully</p>
        </div>
        ...
        <div v-else class="overflow-hidden rounded-xl border border-stone-200 dark:border-stone-800">
          <table class="w-full">
            <thead class="bg-stone-50 dark:bg-stone-800/50">
              <tr>
                <th class="px-4 py-3 text-left text-xs font-medium text-stone-500 dark:text-stone-400 uppercase tracking-wider">Email</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-stone-500 dark:text-stone-400 uppercase tracking-wider">Reason</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-stone-500 dark:text-stone-400 uppercase tracking-wider">Detail</th>

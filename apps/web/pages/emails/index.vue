<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-display text-stone-900 dark:text-stone-100">Emails</h1>
        <p class="mt-1 text-stone-500 dark:text-stone-400">View and track all your sent emails.</p>
      </div>
      <span v-if="emails" class="text-sm text-stone-500 dark:text-stone-400">{{ formatNumber(emails.total) }} total</span>
    </div>
    ...
    <div class="card-elevated p-6">
      ...
      <template v-else-if="emails">
        <div v-if="emails.items.length === 0" class="empty-state">
          <div class="empty-state-icon">
            <UIcon name="i-heroicons-envelope" class="w-8 h-8 text-stone-400" />
          </div>
          <p class="text-stone-500 dark:text-stone-400">No emails found</p>
        </div>
